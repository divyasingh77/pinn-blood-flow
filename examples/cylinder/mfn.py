...
from modulus.sym.models.multiplicative_filter_net import MultiplicativeFilterNetArch
...
def run(cfg: ModulusConfig) -> None:
    ...
    outlet_mesh = Tessellation.from_stl(
        point_path + "/outlet.stl", airtight=False
    )
    noslip_mesh = Tessellation.from_stl(
        point_path + "/wall.stl", airtight=False
    )
    ...
    # scale and normalize mesh and modsim data
    center = (2.5, 0, 0)
    scale = 1  
    ...
    outlet_normal = (-1, 0, 0)
    outlet_area = (0.5**2 * np.pi) * (scale**2)
    outlet_radius = 0.5
